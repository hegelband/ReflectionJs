...
const findArgsSeparatorCommas = (str) => {
    const allCommas = str.split('').map((char, index) => ({ char, index })).filter(s => s.char === ',');
    let prevCommaIndex = 0;
    const argsSeparatorCommas = allCommas.filter(comma => {
        const openBracesCount = str.slice(prevCommaIndex, comma.index).split('').filter(s => s === '(').length;
        const closeBracesCount = str.slice(prevCommaIndex, comma.index).split('').filter(s => s === ')').length;
        const openCurlyBracesCount = str.slice(prevCommaIndex, comma.index).split('').filter(s => s === '{').length;
        const closeCurlyBracesCount = str.slice(prevCommaIndex, comma.index).split('').filter(s => s === '}').length;
